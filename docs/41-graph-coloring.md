...
집합 객체에 소속된 구성원이 서로 인접할 수 없는 상황을 포함된다. 
예를 들어, 수업 시간표나 회의  일정표를 작성하는 사람은 관련된 
모든 사람의 제약조건을 만족시키는 문제에 맞닥드리게 된다. 
...
## 그래프 색칠하기 {#graph-coloring-intro}

![지도 색칠](img/ch13-coloring/13-coloring-01-painter.png){width=100%}

### 들어가며

이번 활동은 이야기를 중심으로 돌아간다. 학생이 지도에 있는 나라를 색칠하는 지도 제작자를 돕는다. 
국경을 맞대고 있는 나라가 다른 색깔이기만 하면, 
나라를 무슨 색으로 칠하든지 문제가 되지 않는다.  
...
(만약 두 나라가 한 점에서 만난다면, 국경을 공유하는 것으로 간주되지 않아서, 
동일한 색깔을 칠할 수 있다.) Southland를 빨간색으로 색칠해서 
지도상에 단지 두가지 색으로 색칠을 끝마칠 수 있다.  

지도 제작자가 가난해서, 크래용을 많이 사용할 수 없다. 
그래서, 가능한 적은 수의 크래용을 사용하는 아이디어를 내보자.  

![4 대륙](img/ch13-coloring/13-coloring-02-four-countries.png){width=100%}
...
## 활동: 그래프 색칠하기 1 {#graph-coloring-worksheet-01}

가능하면 적은 수의 색을 사용하여 지도에 있는 나라들에 색을 칠하세요. 하지만, 국경을 맞대고 있는 인접한 어떠한 나라도 동일한 색깔이면 되지 않도록 확실히 하세요.  

![](img/ch13-coloring/13-coloring-03-coloring-countries.png){width=100%}

## 활동: 그래프 색칠하기 2 {#graph-coloring-worksheet-02}

가능하면 적은 수의 색을 사용하여 지도에 있는 나라들에 색을 칠하세요. 하지만, 국경을 맞대고 있는 인접한 어떠한 나라도 동일한 색깔이면 되지 않도록 확실히 하세요.   

![](img/ch13-coloring/13-coloring-04-coloring-countries.png){width=100%}

## 활동: 그래프 색칠하기 3 {#graph-coloring-worksheet-03}

가능하면 적은 수의 색을 사용하여 지도에 있는 나라들에 색을 칠하세요. 하지만, 국경을 맞대고 있는 인접한 어떠한 나라도 동일한 색깔이면 되지 않도록 확실히 하세요.  

![](img/ch13-coloring/13-coloring-05-coloring-countries.png){width=100%}



## 활동: 그래프 색칠하기 4 {#graph-coloring-worksheet-04}

가능하면 적은 수의 색을 사용하여 지도에 있는 나라들에 색을 칠하세요. 하지만, 국경을 맞대고 있는 인접한 어떠한 나라도 동일한 색깔이면 되지 않도록 확실히 하세요.  

![나라 색칠](img/ch13-coloring/13-coloring-06-coloring-countries.png){width=100%}
...
## 컴퓨터 과학 핵심 개념 {#graph-coloring-lesson}
...
![4 대륙](img/ch13-coloring/13-coloring-07-four-lands.png){width=100%}
...
![과목 선택](img/ch13-coloring/13-coloring-07-school-subject.png){width=100%} 
...
## 해답과 힌트 {#graph-coloring-solution}

워크쉬트 1 에 있는 지도에 대한 유일한 해답이다. (물론, 색깔을 선택하는 것은 학생에게 달려있지만, 두가지 다른 색만 필요하다.)  

![1번 해답](img/ch13-coloring/13-coloring-08-solution.png){width=100%}

워크쉬트 2 의 상단에 있는 지도는 세가지 색으로 채색될 수 있는 반면에 하단에 있는 지도는 네가지 색이 필요하다. 여기 두가지 가능한 해답이 있다.

![2번 해답](img/ch13-coloring/13-coloring-09-solution.png){width=100%}

워크쉬트 3 에 있는 지도는 좀더 단순한 세가지 색깔지도가 되고 가능한 정답이 함께 있다.

![3번 해답](img/ch13-coloring/13-coloring-10-solution.png){width=100%}

단지 색깔 두개만 사용한 워크쉬트 4 정답은 다음과 같다.

![4번 해답](img/ch13-coloring/13-coloring-11-solution.png){width=100%}
